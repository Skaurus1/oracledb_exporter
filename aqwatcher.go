...
func rowsCount (db *sql.DB, tablename string, qname string) (float64) {

	var rows_count float64
	query := fmt.Sprintf("select count(*) from %s a where a.q_name = %s", tablename, qname)
...
func main() {
	...
	oracleAQName := getenvStr("OracleAQName")
	...
	go func() {
		for {
			count.Set(rowsCount(db, oracleTableName, oracleAQName))
